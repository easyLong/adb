...
更完整的端到端流程见 [PROJECT_FLOW.md](PROJECT_FLOW.md)。本文只说明代码分层和扩展边界。
...
| 工作流层 | `workflows/` | 编排 fetch、initial_check、detail_crawl、excel-detail |
...
行号、文件路径、sheet 名、输出路径属于定位信息，放入 `source_locator_json`，不参与任务身份。

## 手机采集边界

`mobile/` 只提供通用执行能力，不理解具体 App 业务。
