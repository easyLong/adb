...
function Load-ProjectEnv {
    ...
    foreach ($line in Get-Content -LiteralPath $ResolvedPath) {
        if ($line -match '^\s*(?:\$env:)?(\uFEFF?(?:MYSQL|OPENAI|WECHAT|SCHEDULER|DEVICE|CAPTURE)_[A-Z0-9_]+)\s*=\s*(.*)\s*$') {
...
function Get-QueueWorkerDefinitions {
    @(
        ...
        [pscustomobject]@{ Name = "wechat"; Roles = "wechat" }
    )
}
...
if ($Task -eq "scheduler") {
    ...
    if ($WechatPages) {
        $OnceArgs += @("--wechat-pages", $WechatPages)
    }
    if ($WechatOutDir) {
        $OnceArgs += @("--wechat-out-dir", $WechatOutDir)
    }
    if ($WechatSerial) {
        $OnceArgs += @("--wechat-serial", $WechatSerial)
    }
    if ($WechatLimit) {
    ...
    if ($WechatSkipNavigation) {
        $OnceArgs += "--wechat-skip-navigation"
    }
    if ($WechatKeepOnDevice) {
        $OnceArgs += "--wechat-keep-on-device"
    }
    foreach ($Item in $ConfigSet) {
